...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  const row = await db.prepare(
      "SELECT brand_id, to_char(report_date, 'YYYY-MM-DD') AS report_date FROM sales_live WHERE id = ?"
    ).get<{ brand_id: number | null; report_date: string }>(id);
  await db.prepare("DELETE FROM sales_live WHERE id = ?").run(id);
  if (row) {
    await db.prepare(
        "DELETE FROM sales_live_campaign WHERE marketer_id = ? AND brand_id IS NOT DISTINCT FROM ? AND report_date = ?"
